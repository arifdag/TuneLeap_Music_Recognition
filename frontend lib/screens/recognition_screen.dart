...
class _RecognitionScreenState extends State<RecognitionScreen> with TickerProviderStateMixin {
  ...
  void _startRecording() async {
    ...
    _recordingTimer = Timer.periodic(Duration(seconds: 1), (timer) {
      setState(() {
        _recordingSeconds++;
        _recordingLevel = 0.3 + (Random().nextDouble() * 0.7);
      });
                ...
                AnimatedBuilder(
                  ...
                  builder: (context, child) {
                    return Transform.scale(
                      scale: _isRecording ? widget.pulseAnimation.value : 1.0,
                      child: Container(
                        width: 200,
                        height: 200,
      ...
      builder: (context, child) {
        return Row(
          ...
          children: List.generate(5, (index) {
            double height = 4 + (sin((_waveAnimation.value * 2 * pi) + (index * 0.5)) * 20 * _recordingLevel);
            return Container(
