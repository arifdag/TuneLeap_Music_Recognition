...
class DatabaseHelper {
  ...
  Future<Database> _initDB() async {
    ...
    return await openDatabase(
      path,
      version: 1, // You might want to increment version if schema changes from a previous state
      onCreate: _onCreate,
      // onUpgrade: _onUpgrade, // Implement if you plan schema migrations
    );
  }
  ...
  Future _onCreate(Database db, int version) async {
    ...
    ''');
    // Removed RecognitionHistory, UserPlaylists, PlaylistSongs table creations
  }

  /*
  // Optional: If you ever change the UserSettings schema or add new tables later.
  Future _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < newVersion) {
      // Handle schema upgrades here if UserSettings table structure changes in the future
      // For example:
      // if (oldVersion < 2) {
      //   await db.execute("ALTER TABLE UserSettings ADD COLUMN newColumn TEXT;");
      // }
    }
  }
  */
