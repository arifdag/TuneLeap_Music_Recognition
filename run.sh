...
celery -A worker.tasks.celery_app worker --loglevel=info -c 2 &

# Start the Gunicorn web server in the foreground.
# This will be the main process for the Render service.
echo "Starting Gunicorn server..."
exec gunicorn -k uvicorn.workers.UvicornWorker -w "${WEB_CONCURRENCY:-1}" -b "0.0.0.0:${PORT}" --log-level "${LOG_LEVEL:-info}" api.main:app